...
function appendDenseParallelInstruction(slice, targetRows) {
  ...
  if (isCostBreakdownLikeSlice(slice)) {
    parts.push('Cost-breakdown numeric integrity contract: a multi-row cost table with a per-row formula (e.g. Importo = Quantity * UnitCost) requires EVERY row to share the SAME formula shape. Mixing absolute amounts (€) and percentages (%) under one multiplication produces values orders of magnitude wrong (e.g. a %-row whose UnitCost column holds 0.02 multiplied by a Quantity that another row uses as a € base yields €-billion garbage that cascades into every downstream sheet). For percentage-style costs: either (a) precompute the absolute € in Assumptions and reference it as Unit="€", Quantity=1, UnitCost=<computed>, OR (b) write =base*pct explicitly on that row. Sanity guardrail: before calling done, verify no single cost line exceeds roughly 3× the largest single hard-cost input (terreno, costruzione totale) unless it is an explicit grand total.');
  }
  return parts.join('\n').slice(0, 8000);
}
...
- A guard rejects any write where the same scalar value appears in ≥20 distinct rows of one column — that is filler padding, not real data, and it pollutes downstream SUMIFs.
- Cash Flow MUST be 1 row per period across the full horizon (e.g. 1 row per month for 36 months × 10 piani = 360 rows, OR a wide pivot of months as columns × line items as rows). Do NOT write sentinel rows at the section boundaries only and leave the rest blank.
...
- ZERO-DETERMINISTIC POLICY (this is a valuation/returns slice). You MUST locate the following inputs on Assumptions BEFORE writing any IRR/NPV/WACC/DSCR formula:
  ...
  Fabricating fallback numbers for missing valuation inputs produces computed-looking but nonsensical outputs (negative WACC, negative DSCR, IRR in the hundreds) and leaves the user no signal that the inputs are missing. NEVER do this — surface the gap explicitly.`
    : '';
...
- TRUST YOUR WRITES — DO NOT RE-READ CELLS YOU JUST WROTE. set_cell_range / bulk_set_cell_ranges are durable; the ACK returned by the harness is authoritative. Once you write A1=42, A1 IS 42 until you overwrite it. Read ONLY upstream sheets owned by OTHER slices (from may_read_from). If a write returned an error in the ACK (#VALUE!/#REF!), fix the formula directly — no re-read needed to confirm "did it actually write?".${hasUpstream ? `
- READ BEFORE YOU WRITE: your first tool call MUST be a read (get_cell_ranges / read_sheet) against the upstream ranges listed above. Do NOT guess upstream layout from the slice instructions — workers that skip this step write formulas pointing to wrong cells and have to redo the slice. Confirm exact addresses, then write your formulas against those exact addresses.
